...
class BodyLayout extends State<StatoAttivitaPazienti> {
  ...
  // ),
      body: Center(
                         child: StreamBuilder(
                          stream: FirebaseFirestore.instance.collection("patients").orderBy("cognome").snapshots(),
                              ...
                              children: snapshot.data!.docs.map((patient){
                                return Center(child: 
                                    ListTile(
                                      ...
                                      subtitle: Text(patient['status']=='online' ? 'online' : 'ultimo accesso: ' + patient['ultimo_accesso']),
                                      trailing: Icon(Icons.circle, color: patient['status'] == 'online' ? Colors.greenAccent : Colors.redAccent,),
                                ),
                                );
                              }).toList(),
                            );
